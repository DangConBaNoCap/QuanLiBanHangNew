...
namespace QuanLiBanHang.Gui
{
    public partial class frmBanHang : Form
    {
        ...
        private void toolTimTen_TextChanged(object sender, EventArgs e)
        {
            string dk=toolTimTen.Text.ToString();
            if (dk != string.Empty)
                dgvHanghoa.DataSource = hd.BangHangHoaBan("where TenSP like N'%" + dk.Trim() + "%' ");
            else dgvHanghoa.DataSource = hd.BangHangHoaBan("");
        }

        private void toolTimNSX_TextChanged(object sender, EventArgs e)
        {
            string dk = toolTimNSX.Text.ToString();
            if (dk != string.Empty)
                dgvHanghoa.DataSource = hd.BangHangHoaBan("where DiaChi like N'%" + dk.Trim() + "%' ");
            else dgvHanghoa.DataSource = hd.BangHangHoaBan("");
        }

        private void toolTimSL_TextChanged(object sender, EventArgs e)
        {
            string dk = toolTimSL.Text.ToString();
            if (dk != string.Empty)
                dgvHanghoa.DataSource = hd.BangHangHoaBan("where SoLuong like N'%" + dk.Trim() + "%' ");
            else dgvHanghoa.DataSource = hd.BangHangHoaBan("");
        }

        private void toolTimDG_TextChanged(object sender, EventArgs e)
        {
            string dk = toolTimDG.Text.ToString();
            if (dk != string.Empty)
                dgvHanghoa.DataSource = hd.BangHangHoaBan("where GiaBan like N'%" + dk.Trim() + "%' ");
            else dgvHanghoa.DataSource = hd.BangHangHoaBan("");
        }

        private void toolTimMa_TextChanged(object sender, EventArgs e)
        {
            string dk = toolTimMa.Text.ToString();
            if (dk != string.Empty)
                 dgvHanghoa.DataSource = hd.BangHangHoaBan("where MaSP like N'%" + dk.Trim() + "%' ");
             else dgvHanghoa.DataSource = hd.BangHangHoaBan("");
        }
